...
A **source-free series $RLC$ circuit** occurs when its external independent sources are disconnected for $t > 0$. The circuit's response depends entirely on the energy initially stored in the capacitor (initial voltage $V_0$) and the inductor (initial current $I_0$).
